...
public class DigitalSignatureTest {
    ...
    @Test
    public void testKeysExist() throws CryptoException {
    ...
    public void testSignVerify() throws CryptoException {
        ...
        boolean verified = DigitalSignature.verify64(keys.getPublic(), "hello", signed);
        assertTrue(verified);
    }

    @Test
    public void testSignVerifyBytes() throws CryptoException {
        KeyPair keys = DigitalSignature.generateKeys();

        byte[] signed = DigitalSignature.sign(keys.getPrivate(), "hello");
        boolean verified = DigitalSignature.verify(keys.getPublic(), "hello", signed);
        assertTrue(verified);
    }

    @Test
    public void testSignVerifyLongData() throws CryptoException {

        KeyPair keys = DigitalSignature.generateKeys();

        String signed = DigitalSignature.sign64(keys.getPrivate(), BYTES_3K);
        boolean verified = DigitalSignature.verify64(keys.getPublic(), BYTES_3K, signed);
        assertTrue(verified);
    }

    @Test
    public void testPersistKeysLoadPrivate() throws CryptoException {

        KeyPair keys = DigitalSignature.generateKeys();
        String signed = DigitalSignature.sign64(keys.getPrivate(), "hello");

        DigitalSignature.persist(keys, Paths.get(TEST_RESOURCES_PATH + "key"), Paths.get(TEST_RESOURCES_PATH + "certificate"));

        PrivateKey privKey = DigitalSignature.getPrivateKey(Paths.get(TEST_RESOURCES_PATH + "key.key"));
        String reSigned = DigitalSignature.sign64(privKey, "hello");

        assertEquals(reSigned, signed);
    }

    @Test
    public void testPersistKeysLoadPublic() throws CryptoException {
        ...
        PublicKey publicKey = DigitalSignature.getCertificate(Paths.get(TEST_RESOURCES_PATH + "certificate.crt"));
